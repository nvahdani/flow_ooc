...
#PCA for each timepoint- revisions request
counts110_trimmed_colremove_4h <- counts110_trimmed_colremove[,c(3,4,7,8,11,12)]
counts110_trimmed_colremove_24h <- counts110_trimmed_colremove[,c(1,2,5,6,9,10)]


PCA(counts110_trimmed_colremove_4h,
    shape_pca = factor(new_metadata$flow_profile), save_plot = "no",
    name_of_plot = "PCA_nolog")

PCA(counts110_trimmed_colremove_24h,
    shape_pca = factor(new_metadata$flow_profile), save_plot = "no",
    name_of_plot = "PCA_nolog")
...
GO_res_uni_bidirectional_24_ <- 
  read.csv("/Users/negarvahdani/Deseq2/pump_tilter_24h_GO_BP_pathways.csv") 
...
GO_res_uni_bidirectional_4_ <- 
  read.csv("/Users/negarvahdani/Deseq2/pump_tilter_4h_GO_BP_pathways.csv") 
#make a list of the select the pathways of interest
...
# create a dotplot for the GO terms with GeneRatio on x axis decreasing
...
# Plot Heatmap for figure 2

```{r}
#| label: Heatmap
#| include: true
# create a variance stabilized data
vst_data <- vst(dds_flow_time, blind = FALSE) 
# create a matrix of the variance stabilized data
heatmap_matrix <- assay(vst_data) 
degs <- rbind(res_uni_bidirectional_4_padj0.05,
              res_uni_bidirectional_24_padj0.05)
degs %>% head %>% print
heatmap_matrix_degs <- heatmap_matrix[rownames(heatmap_matrix) 
                                      %in% rownames(degs), ]

# Step 2: Calculate z-scores for the DEGs
heatmap_matrix_degs_z <- t(scale(t(heatmap_matrix_degs)))
# subset the heatmap matrix to the selected columns
heatmap_matrix_all_subset_col <- 
  heatmap_matrix_degs_z[, c("e3_4h_t","e4_4h_t","e6_4h_t","e3_4h_p",
                         "e4_4h_p","e6_4h_p","e3_24h_t","e4_24h_t","e6_24h_t",
                         "e3_24h_p","e4_24h_p","e6_24h_p")] 
...
Heatmap(heatmap_matrix_all_subset_col, name="Expression Levels", 
        ...
        heatmap_legend_param = list(
          title = "Expression Levels (z-score)",
          title_gp = gpar(fontsize = 10),
          labels_gp = gpar(fontsize = 10),
          grid_width	= unit(5,"point"),
          grid_height	= unit(5,"point")
...
# Plot Heatmap for revision- selected pathways for 24h time point

```{r}
#| label: heatmap of selected pathways and DEGs
#| include: true
...
head(heatmap_matrix_merged) %>% print
heatmap_matrix_merged_subset24 <- 
  heatmap_matrix_merged[, c("e3_24h_t","e4_24h_t","e6_24h_t",
                         "e3_24h_p","e4_24h_p","e6_24h_p",
                         "hgnc_symbol","ensembl_gene_id")]
head(heatmap_matrix_merged_subset24) %>% print
dim(heatmap_matrix_merged_subset24)
# remove the NAs and the empty strings

heatmap_matrix_merged_subset24_cleaned <-
  heatmap_matrix_merged_subset24[!is.na(heatmap_matrix_merged_subset24$
                                          hgnc_symbol), ]
heatmap_matrix_merged_subset24_cleaned <-
  heatmap_matrix_merged_subset24_cleaned %>% filter(hgnc_symbol != "")
heatmap_matrix_merged_subset24_cleaned %>% filter(hgnc_symbol == "") %>% dim 
dim(heatmap_matrix_merged_subset24_cleaned)
sum(is.na(heatmap_matrix_merged_subset24_cleaned$hgnc_symbol))

# check for duplicates
duplicate <- 
  heatmap_matrix_merged_subset24_cleaned[
    duplicated(heatmap_matrix_merged_subset24_cleaned$hgnc_symbol)
                                         |duplicated(
                                           heatmap_matrix_merged_subset24_cleaned$hgnc_symbol,
                                                     fromLast=TRUE),]
df_filtered <- heatmap_matrix_merged_subset24_cleaned %>% 
  group_by(hgnc_symbol) %>% 
  rowwise() %>%
  filter(n_distinct(c_across(c("e3_24h_t","e4_24h_t","e6_24h_t",
                         "e3_24h_p","e4_24h_p","e6_24h_p"))) > 1) %>%
  ungroup()

df_duplicated <- 
  df_filtered[duplicated(df_filtered$hgnc_symbol)
              |duplicated(df_filtered$hgnc_symbol, fromLast=TRUE),]

# List of Ensembl Gene IDs to exclude from the DataFrame
gene_ids_to_exclude <- c("ENSG00000161912","ENSG00000291140",
                         "ENSG00000292339", "ENSG00000124549",
                         "ENSG00000205771", "ENSG00000154874",
                         "ENSG00000292362", "ENSG00000250959",
                         "ENSG00000258137", "ENSG00000292358",
                         "ENSG00000206149", "ENSG00000258724",
                         "ENSG00000168255", "ENSG00000292327",
                         "ENSG00000205763", "ENSG00000189136",
                         "ENSG00000259658", "ENSG00000259429", 
                         "ENSG00000124333", "ENSG00000292372", 
                         "ENSG00000214717","ENSG00000197083",
                         "ENSG00000198155", "ENSG00000228903",
                         "ENSG00000214331")
# Remove rows based on the column ensembl_gene_id
df_filtered <- 
  df_filtered[!df_filtered$ensembl_gene_id %in% gene_ids_to_exclude, ]
df_duplicated_after <- 
  df_filtered[duplicated(df_filtered$hgnc_symbol)
              |duplicated(df_filtered$hgnc_symbol, fromLast=TRUE),]
...
#set row names using the hgnc_symbol column
rownames(df_filtered) <- df_filtered$hgnc_symbol
df_filtered$hgnc_symbol <- NULL


degs_24 <- res_uni_bidirectional_24merged_padj0.05
dim(degs_24)
degs_cleaned <- degs_24[!duplicated(degs_24$hgnc_symbol),]#
dim(degs_cleaned)
#remove the missing values in degs_clenaed
degs_cleaned <- degs_cleaned[!is.na(degs_cleaned$hgnc_symbol), ]
rownames(degs_cleaned) <- degs_cleaned$hgnc_symbol

heatmap_matrix_degs_24 <- df_filtered[rownames(df_filtered)
                                      %in% rownames(degs_cleaned), ]
head(heatmap_matrix_degs_24) %>% print


genes_to_annotate <- c('SELE', 'KITLG', 'STK10', 'TGFB2', 'HMOX1', 'FLT1',
                       'IL1A', 'PGF','TNFSF18', 'EDNRB', 'IL33', 'CH25H',
                       'LYST', 'ADAM17', 'VCAM1', 'CXCL5','ITGA2', 'LGALS9',
                       'SELP', 'ANO6', 'GBA1', 'CALR', 'CSF1', 'SELL', 
                       'HMGB1', 'CCL23', 'CCL16', 'CFLAR', 'CEACAM1', 'NOX4',
                       'FAM83D', 'HMGCR','GBP1', 'DUSP4', 'BMP4', 'SPRY2',
                       'ACKR3', 'MFHAS1', 'CNKSR3', 'ATF3','NEK10', 'BMPER',
                       'CDK1', 'TNFAIP8L3', 'IQGAP3', 'MIR126', 'ANLN', 'PTGS2',
                       'ANGPT2', 'RGCC', 'SASH1', 'PIK3R3', 'PROX1', 'NR4A1',
                       'EFNB2', 'MACF1','KLF4', 'HAS2', 'NANOS1', 'FGF16',
                       'E2F2', 'RECK', 'E2F8', 'TSPAN18','E2F7', 'AURKA',
                       'PCNA', 'POSTN', 'LPIN1', 'NREP', 'ATIC', 'CCNA2','ADM',
                       'NNMT', 'TYMS', 'DHFR', 'SPAAR', 'TACC3', 'NAV3', 'SPAG5',
                       'MAP2', 'TPX2', 'STMN1', 'KIF18A', 'STIL', 'HAUS8',
                       'PSRC1', 'SKA1','SKA3', 'PLK1', 'FAM107A', 'NUP62', 
                       'BRCA1', 'HIPK2', 'F3', 'AGO2','ADM2', 'ST6GAL1',
                       'UNC13D','MERTK', 'AKIP1', 'PEAK1', 'S100A10', 'OAS1',
                       'OAS3', 'IL1RL1', 'KIF2C', 'NCKAP5', 'KIF18B')

row_annotation <- 
  rowAnnotation(mark = anno_mark(at = which(rownames(heatmap_matrix_degs_24) 
                                            %in% genes_to_annotate),
                                                   labels =
                                   rownames(heatmap_matrix_degs_24)
                                 [rownames(heatmap_matrix_degs_24)
                                   %in% genes_to_annotate],
                                                   labels_gp 
...
heatmap_matrix_degs_24 <-
  subset(heatmap_matrix_degs_24,
         select = -ensembl_gene_id)
heatmap_matrix_degs_24 <- as.matrix(heatmap_matrix_degs_24)
heatmap_matrix_degs_24 <- t(scale(t(heatmap_matrix_degs_24)))

# check how many NAs are there
sum(is.na(heatmap_matrix_degs))
...
Heatmap(heatmap_matrix_degs_24, name="Expression Levels", 
        cluster_columns = FALSE,  
        cluster_rows = TRUE,
        clustering_distance_columns = "pearson",
        clustering_method_columns = "complete",
...
# Plot Heatmap for revision- selected pathways for 4h time point -

```{r}
#| label: heatmap of selected pathways and DEGs 4h
#| include: true
...
head(heatmap_matrix_merged) %>% print
 # subset the heatmap matrix to the selected columns
heatmap_matrix_merged_subset4 <-
  heatmap_matrix_merged[, c("e3_4h_t","e4_4h_t","e6_4h_t",
                         "e3_4h_p","e4_4h_p","e6_4h_p",
                         "hgnc_symbol","ensembl_gene_id")]
head(heatmap_matrix_merged_subset4) %>% print
dim(heatmap_matrix_merged_subset4)
heatmap_matrix_merged_subset4_cleaned <-
  heatmap_matrix_merged_subset4[!is.na(heatmap_matrix_merged_subset4$hgnc_symbol)
                                , ]
heatmap_matrix_merged_subset4_cleaned <-
  heatmap_matrix_merged_subset4_cleaned %>% filter(hgnc_symbol != "")
dim(heatmap_matrix_merged_subset4_cleaned)
sum(is.na(heatmap_matrix_merged_subset4_cleaned$hgnc_symbol))
heatmap_matrix_merged_subset4_cleaned %>% filter(hgnc_symbol == "") %>% dim
duplicate_4 <- 
  heatmap_matrix_merged_subset4_cleaned[
    duplicated(heatmap_matrix_merged_subset4_cleaned$hgnc_symbol)
                                        |duplicated(
                                          heatmap_matrix_merged_subset4_cleaned$
                                            hgnc_symbol,
                                                    fromLast=TRUE),]

df_filtered_4 <- heatmap_matrix_merged_subset4_cleaned %>% 
  group_by(hgnc_symbol) %>% 
  rowwise() %>%
  filter(n_distinct(c_across(c("e3_4h_t","e4_4h_t","e6_4h_t",
                         "e3_4h_p","e4_4h_p","e6_4h_p"))) > 1) %>%
  ungroup()
df_filtered_4 %>% dim
df_duplicated_4 <- 
  df_filtered_4[duplicated(df_filtered_4$hgnc_symbol)
                |duplicated(df_filtered_4$hgnc_symbol,
                            fromLast=TRUE),]

# List of Ensembl Gene IDs to exclude from the DataFrame
gene_ids_to_exclude_4 <- c("ENSG00000161912","ENSG00000291140",
                           "ENSG00000292339", "ENSG00000124549",
                           "ENSG00000205771", "ENSG00000154874",
                         "ENSG00000292362", "ENSG00000250959",
                         "ENSG00000258137", "ENSG00000292358",
                         ...
                         "ENSG00000124333", "ENSG00000292372",
                         "ENSG00000214717","ENSG00000197083",
                         "ENSG00000198155","ENSG00000225492",
                         "ENSG00000238105","ENSG00000243753",
                         "ENSG00000236871","ENSG00000292342",
                         "ENSG00000182378","ENSG00000292334",
                         "ENSG00000228903", "ENSG00000214331")
heatmap_matrix_merged_subset4_cleaned <- 
  heatmap_matrix_merged_subset4_cleaned[!heatmap_matrix_merged_subset4_cleaned$
                                          ensembl_gene_id
                                        %in% gene_ids_to_exclude, ]
df_filtered_4 <-
  df_filtered_4[!df_filtered_4$ensembl_gene_id
                %in% gene_ids_to_exclude_4, ]
df_duplicated_after_4 <- 
  df_filtered_4[duplicated(df_filtered_4$hgnc_symbol)
                |duplicated(df_filtered_4$hgnc_symbol,
                            fromLast=TRUE),]
# Convert tibble to a data frame
df_filtered_4 <- as.data.frame(df_filtered_4)

# Set row names using the hgnc_symbol column
rownames(df_filtered_4) <- df_filtered_4$hgnc_symbol

# Optionally, you can remove the hgnc_symbol column if you don't need it anymore
df_filtered_4$hgnc_symbol <- NULL


degs_4 <- res_uni_bidirectional_4hmerged_padj0.05
dim(degs_4)
degs_cleaned_4 <- degs_4[!duplicated(degs_4$hgnc_symbol),]
dim(degs_cleaned_4)
degs_cleaned_4 <- na.omit(degs_cleaned_4)
rownames(degs_cleaned_4) <- degs_cleaned_4$hgnc_symbol

heatmap_matrix_degs_4_ano <-
  df_filtered_4[rownames(df_filtered_4) %in% rownames(degs_cleaned_4), ]
heatmap_matrix_degs_4_ano <- as.data.frame(heatmap_matrix_degs_4_ano)


genes_to_annotate_4 <- c("TAB2", "SPHK2", "HMOX1", "NFKB1",
...
row_annotation_4 <-
  rowAnnotation(mark = anno_mark(at = which(rownames(heatmap_matrix_degs_4_ano)
                                            %in% genes_to_annotate_4),
                                                   labels = 
                                   rownames(heatmap_matrix_degs_4_ano)
                                 [rownames(heatmap_matrix_degs_4_ano)
                                   %in% genes_to_annotate_4],
                                                   labels_gp = 
                                   gpar(fontsize = 10, fontface = "bold"),
                                                   padding = unit(2, "mm")))
heatmap_matrix_degs_4_ano <-
  subset(heatmap_matrix_degs_4_ano,
         select = -ensembl_gene_id)
heatmap_matrix_degs_4_ano <-
  heatmap_matrix_degs_4_ano[, !names(heatmap_matrix_degs_4_ano)
                            %in% "hgnc_symbol", drop = FALSE]

heatmap_matrix_degs_4_ano <- na.omit(heatmap_matrix_degs_4_ano)
heatmap_matrix_degs_4_ano <- as.matrix(heatmap_matrix_degs_4_ano)
heatmap_matrix_degs_4_ano <- t(scale(t(heatmap_matrix_degs_4_ano)))

# check how many NAs are there
sum(is.na(heatmap_matrix_degs_4_ano))
ha = HeatmapAnnotation(
...
Heatmap(heatmap_matrix_degs_4_ano, name="Expression Levels", 
        cluster_columns = FALSE,  
        cluster_rows = TRUE,
        clustering_distance_columns = "pearson",
        clustering_method_columns = "complete",
        ...
        row_title_rot = 0,
        bottom_annotation = ha,
        right_annotation = row_annotation_4,
        width = unit(8, "cm"),
        height = unit(10, "cm"),
        heatmap_legend_param = list(
          title = "Expression Levels (z-score)",
          title_gp = gpar(fontsize = 10),
          labels_gp = gpar(fontsize = 10),
          grid_width	= unit(4,"point"),
          grid_height	= unit(4,"point")
        ),
...
```{r}
#save.image("workspace/Deseq_cleanedup_analysis_160224.Rdata")
```
