...
count_elife <-read.delim("/Users/negarvahdani/elife_reverse_strand_gene_counts_inforemoved.txt",sep = "\t", stringsAsFactor = FALSE, header= TRUE, fill = TRUE, row.names = 1)
colnames(count_elife) <- gsub("X.data.users.nvahdani.flow_project.meta.analysis.bamsort.", "", colnames(count_elife))
...
#load the metadata
metadata_combined <- read.csv("results/metadata_combined_elife.csv", row.names = 1) # available on github
...
missing_rows <- rownames(count_elife)[!rownames(count_elife) %in% rownames(counts110_trimmed_colremove)] 
...
batchcorrected_counts <- ComBat_seq(as.matrix(counts_combined),batch = metadata_combined$batch)  
...
PCA(assay(dds_combined_filtered_rlog), color_pca = factor(metadata_combined$flow_profile), shape_pca = factor(metadata_combined$batch), save_plot = "no", name_of_plot = "PCA_counts_batch_corrected_flow_profile_batch_nona_rlog_afterfiltering")
...
##tilter 4h vs cord
res_tilter4_cord <- lfcShrink(dds_combined_filtered,parallel = TRUE, contrast=c("flow_time", "bidirectional_4","in_vivo_in_vivo"), type="ashr")
...
##tilter 24h vs cord
res_tilter24_cord <- lfcShrink(dds_combined_filtered,parallel = TRUE, contrast=c("flow_time", "bidirectional_24","in_vivo_in_vivo"), type="ashr")
...
##pump 4h vs cord
res_pump4_cord <- lfcShrink(dds_combined_filtered,parallel = TRUE, contrast=c("flow_time", "unidirectional_4","in_vivo_in_vivo"), type="ashr")
...
##pump 24h vs cord
res_pump24_cord <- lfcShrink(dds_combined_filtered,parallel = TRUE, contrast=c("flow_time", "unidirectional_24","in_vivo_in_vivo"), type="ashr")
...
symbol_to_ensmbl_human <- function() {
  ...
  ensembl <- useMart(biomart = "ensembl", dataset = "hsapiens_gene_ensembl") # use the main Ensembl mart host
  # query biomart
  annotations <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"), mart = ensembl) # get the ensembl gene id and the hgnc symbol
...
res_tilter4_cord <- merge(res_tilter4_cord, gene_id_df,by.x = "ensembl_gene_id",all.x=TRUE)
res_tilter24_cord <- merge(res_tilter24_cord, gene_id_df,by.x = "ensembl_gene_id",all.x=TRUE)
res_pump4_cord <- merge(res_pump4_cord, gene_id_df,by.x = "ensembl_gene_id",all.x=TRUE)
res_pump24_cord <- merge(res_pump24_cord, gene_id_df,by.x = "ensembl_gene_id",all.x=TRUE)
## remove NAs
...
write.csv(as.data.frame(res_tilter4_cord_padj0.05), file="tilter_4_cord_0.05.csv")
write.csv(as.data.frame(res_tilter24_cord_padj0.05), file="tilter_24_cord_0.05.csv")
...
write.csv(as.data.frame(res_pump4_cord_padj0.05), file="pump_4_cord_0.05.csv")
write.csv(as.data.frame(res_pump24_cord_padj0.05), file="pump_24_cord_0.05.csv")
